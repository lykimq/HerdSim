...
COMPARISON_CAVEATS = [
    (
        "Tick semantics differ by method family: Strombom and Flocking Dog use "
    ...
    (
        "Paper preset keeps each method's own agent counts. For fair comparison, "
...
def report_to_csv(payload: dict[str, Any], *, request: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        f"# methods: {', '.join(str(a) for a in package['experiment'].get('methods') or [])}",
...
def report_to_markdown(payload: dict[str, Any], *, request: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        f"- Methods: {', '.join(str(a) for a in exp.get('methods') or [])}",
