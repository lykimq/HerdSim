...
def iter_one_trial(
    *,
    ...
    """Yield start/tick/trial events for one method x seed run."""
    method_id = method
    ...
    # Factor-grid cells can omit method: recover the matching param bundle.
    if not method_id and sheep_model and dog_controller:
        method_id = find_method_for_models(sheep_model, dog_controller)
    if method_id:
        get_method(method_id)
    elif not (sheep_model and dog_controller):
        raise ValueError("Provide method or sheep_model+dog_controller")
    ...
    label_id = method_id or f"{config['sheep_model']}+{config['dog_controller']}"
    ...
    *,
    ...
    if specs:
        if ids and len(ids) != 1:
            raise ValueError("Factor grids with a method require exactly one")
        if not ids:
        ...
        raise ValueError("Provide methods")

    for method_id in ids:
        get_method(method_id)

    method_loop = ids if ids else [None]
    total = len(method_loop) * len(seeds) * len(param_sets)
    rows: list[dict[str, Any]] = []
    index = 0
    for method_id in method_loop:
        for params in param_sets:
            for seed in seeds:
                index += 1
