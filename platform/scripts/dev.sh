...
export PYTHONPATH="${ROOT}:${ROOT}/platform:${ROOT}/scaling${PYTHONPATH:+:$PYTHONPATH}"
...
setsid env PYTHONPATH="$PYTHONPATH" "${UVICORN[@]}" api.main:app --reload --port 8000 </dev/null &
BACKEND_PID=$!
...
setsid npm --prefix "$PLATFORM/frontend" run dev -- --port 5173 --strictPort </dev/null &
FRONTEND_PID=$!
