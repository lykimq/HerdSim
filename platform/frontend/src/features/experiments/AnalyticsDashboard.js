...
import { renderAnalyticsCharts } from "./analyticsCharts.js";
import {
  analyticsChartsHtml,
  analyticsRunnerHtml,
...
const DEFAULT_BENCHMARK_METHOD_IDS = [
  "strombom",
  "kubo",
  "flocking_dog",
  "v_formation",
...
export function createAnalyticsDashboard({
  ...
  sidePanel
    .querySelector(".analytics-tab-bar")
  ...
  const algBlurbEl = runner.querySelector('[data-role="method-blurb"]');
  const defaultMethodIds = new Set(
    DEFAULT_BENCHMARK_METHOD_IDS.filter((id) =>
      methods.some((a) => a.id === id),
    ),
  );
  if (!defaultMethodIds.size && methods[0]?.id) {
    defaultMethodIds.add(methods[0].id);
  }
  algList.innerHTML = methods
    .map((a) => {
      const tip = methodBlurb(a);
      const titleAttr = tip ? ` title="${tip.replace(/"/g, "&quot;")}"` : "";
      const checked = defaultMethodIds.has(a.id) ? "checked" : "";
      return `
  ...
  const defaultScenarioId = scenarios.some(
    (s) => s.id === DEFAULT_BENCHMARK_SCENARIO_ID,
  )
  ...
  function syncContextBlurbs() {
    ...
    const ids = mode === "grid" ? [] : selectedMethodIds();
    const alg = methods.find((a) => a.id === ids[0]);
    const scen = scenarios.find((s) => s.id === scenSelect.value);
    const preset = presetSelect.value;

    if (mode === "grid") {
      algBlurbEl.textContent =
        "Factor grid uses sheep model + dog controller (and matching param bundles). Compare mode selects named methods.";
    } else if (ids.length > 1) {
      algBlurbEl.textContent =
        "Multiple methods selected; each uses its own paper defaults when Mode is Paper original.";
    } else {
      algBlurbEl.textContent = methodBlurb(alg);
    }
    ...
    if (preset === "paper" && mode !== "grid" && ids.length > 1) {
      presetBlurb.textContent =
        "Each selected method runs with its own paper/reference defaults.";
    } else {
  ...
  function setProgress(unitsDone, total, message) {
    const pct =
      total > 0 ? Math.min(100, Math.round((unitsDone / total) * 100)) : 0;
  ...
  async function renderSummary(payload) {
    ...
    if (headlineEl)
      headlineEl.textContent = headlineFromSummary(payload.summary || []);
            ...
            if (event.type === "progress" || event.type === "tick") {
              const tickPart =
                event.tick != null && event.max_ticks != null
                  ...
                  : "";
              const sweepPart = event.sweep_label
                ? ` [${event.sweep_label}]`
                : "";
              setProgress(
                trialUnits(event),
                event.total,
                `Running ${event.index}/${event.total}: ${event.method}${sweepPart} seed ${event.seed}${tickPart} (${elapsed}s)`,
              );
  ...
  results
    ...
    .addEventListener("click", async () => {
      ...
      downloadText(
        `herdsim_benchmark_${Date.now()}.json`,
        body,
        "application/json",
      );
    });
  results
    .querySelector('[data-role="md"]')
    .addEventListener("click", async () => {
      if (!lastPayload) return;
  ...
  async function mount() {
    ...
    if (lastPayload) {
      ...
      setIdleStatus(
        `Showing previous results. ${lastPayload.rows.length} trials.`,
      );
    }
    ...
    for (const alg of methods) {
      let details = alg;
      try {
        details = await fetchMethod(alg.id);
      } catch {
        // ignore
      }
