...
export function analyticsRunnerHtml() {
  ...
  return `
    ...
    <p class="analytics-intro">
      Compare named methods across seeds, or run a factor grid over sheep model,
      dog controller, flock size, and other factors. Matching model pairs reuse
      method param bundles automatically. For fair method ranking, use
    ...
    <div class="control-group">
      ...
      <p class="param-hint">Compare runs several methods on the same seeds. Factor grid sweeps required model/size factors (plus optional axes); no method picker.</p>
    </div>
    <div class="control-group" data-role="compare-algs">
      <label>Methods</label>
      <div class="check-list" data-role="algs"></div>
      <p class="param-hint" data-role="method-blurb"></p>
    </div>
...
export function analyticsResultsHtml() {
  return `
    ...
    <p class="analytics-intro">
      Each row summarizes one method (or factor cell) across the chosen seeds. Success/failure are scenario outcomes;
