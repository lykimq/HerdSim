...
import { emptyStateHtml } from "../../shared/ui/dom.js";
...
function methodOrder(rows, groupKey = "method") {
  return [
    ...new Set(
      rows.map((r) => r[groupKey]).filter((v) => v != null && v !== ""),
    ),
  ];
}

function colorForMethod(methods, methodId) {
  const index = methods.indexOf(methodId);
  return PLOT_COLORS[Math.max(0, index) % PLOT_COLORS.length];
}

function baseLayout(title, yTitle, xTitle = "Method") {
  return {
    title,
...
/**
 * Box plot of a per-trial metric by method (or sweep_label).
 * Failed trials can be excluded from the box and/or marked with X overlays.
 */
export async function renderPlotlyBoxPlot(
  container,
  rawRows,
  key,
  label,
  ...
  const rows = rawRows || [];
  const methods = methodOrder(rows, groupKey);
  if (!methods.length) {
    container.innerHTML = emptyStateHtml("No trial data yet.");
    return;
  }

  const traces = methods.map((alg) => {
    ...
    return {
      ...
      type: "box",
      name: String(alg),
      marker: { color: colorForMethod(methods, alg) },
      boxpoints: false,
    };
  });
...
export async function renderPlotlyPathTicksScatter(container, rawRows) {
  ...
  const methods = methodOrder(rows);
  if (!methods.length) {
    container.innerHTML = emptyStateHtml("No trial data yet.");
    return;
  }
  ...
  const traces = [];
  methods.forEach((alg) => {
    const color = colorForMethod(methods, alg);
    const ok = rows.filter((r) => r.method === alg && r.success);
    const bad = rows.filter((r) => r.method === alg && !r.success);
...
export async function renderPlotlyHerdabilityHeatmap(container, rawRows) {
  ...
  const keys = Object.keys(keyCounts).sort(
    (a, b) => keyCounts[b] - keyCounts[a],
  );
  if (keys.length < 2) {
  ...
  const xKey = keys[0];
  const yKey = keys[1];
  const xVals = [...new Set(parsed.map((r) => String(r.factors[xKey])))].sort(
    compareFactor,
  );
  const yVals = [...new Set(parsed.map((r) => String(r.factors[yKey])))].sort(
    compareFactor,
  );
  const z = yVals.map((y) =>
    xVals.map((x) => {
...
export function summaryRowHtml(row) {
  ...
  const num = (v, digits = 2) =>
    v != null ? Number(v).toFixed(digits) : "n/a";
  return `
    <td>${row.method}</td>
    <td>${row.trials}</td>
...
export function headlineFromSummary(summary = []) {
  ...
  const pct =
    best.success_rate != null
      ? `${(Number(best.success_rate) * 100).toFixed(0)}%`
      : "n/a";
  return `Best success: ${best.method} (${pct} over ${best.trials} trials).`;
}
