...
import { getPresetOption } from "../ui/params.js";
import {
  fmt,
  flockSpreadPhrase,
...
/** Methods that use Collect / Drive (Strombom-family) switching. */
...
function formatSetupValue(value) {
  ...
  if (Array.isArray(value))
    return `[${value.map((v) => formatSetupValue(v) ?? String(v)).join(", ")}]`;
...
export function buildSetupLines({
  ...
  const cfg = config || {};
  const lines = [];
  const methodId = cfg.method || null;
  pushKv(lines, "Method", methodName || methodId);
  if (methodName && methodId && methodName !== methodId) {
    pushKv(lines, "Method id", methodId);
  }
...
export function buildInsightLines({
  ...
  const lines = [];
  const cfg = config || {};
  const id = methodId || cfg.method || null;
  ...
  if (collectDrive && !containment) {
    ...
    if (
      ...
      lines.push(
        "Flock spread decreased over the run with no end outliers, consistent with a successful " +
          "Collect then Drive sequence for this method family.",
      );
    }
    if (
    ...
    if (
      ...
      lines.push(
        "Flock spread increased over the run. For Collect/Drive methods that makes Drive harder, " +
