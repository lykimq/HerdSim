...
export function buildRunReport({
  ...
  const resolvedMethodId = methodId || config?.method || null;
  ...
  const firstGoalTick = firstWhere(
    history,
    (row) => metricAt(row, "sheep_in_goal") > 0,
  )?.tick;
  const allInTick =
    flockSize != null
      ? firstWhere(
          history,
          (row) => metricAt(row, "sheep_in_goal") >= flockSize,
        )?.tick
      : null;
  const ticks = history.length;
  const durationTicks =
    tick != null && first.tick != null
      ? Math.max(0, tick - first.tick)
      : Math.max(0, ticks - 1);
  const pathPerTick =
    path != null && durationTicks > 0 ? path / durationTicks : null;

  const contextBits = [];
  if (methodName) contextBits.push(methodName);
  ...
  if (status === "success") {
    ...
    outcomeLines.push(
      "Scenario success criterion: not met (ran to the tick limit).",
    );
    if (!containment && timeToGoal != null && timeToGoal < 0) {
      outcomeLines.push(
        `Not every sheep reached the ${zone} together during this run.`,
      );
    }
  ...
  if (inGoal != null && flockSize != null) {
    zoneLines.push(
      `Final count in ${zone}: ${fmt(inGoal, 0)} of ${flockSize} sheep.`,
    );
  } else if (inGoal != null) {
  ...
  if (!allInAlreadyTold) {
    const allInAt =
      timeToGoal != null && timeToGoal >= 0 ? timeToGoal : allInTick;
    if (allInAt != null) {
  ...
  if (gcmGoal != null && !containment) {
    zoneLines.push(
      `Final distance from flock centre to goal: ${fmt(gcmGoal)}.`,
    );
  } else if (gcmGoal != null && containment) {
    zoneLines.push(
      `Final distance from flock centre to pen centre: ${fmt(gcmGoal)}.`,
    );
  }
  ...
  if (spread && cohesion != null) {
    flockLines.push(
      `Final flock spread (cohesion): ${fmt(cohesion)} (${spread}).`,
    );
  }
  ...
  if (polarization != null) {
    ...
    if (startPolar != null && Math.abs(polarization - startPolar) >= 1e-6) {
      ...
      motionLines.push(
        `Alignment (polarisation): ${fmt(polarization)} (${align}).`,
      );
    }
  }
