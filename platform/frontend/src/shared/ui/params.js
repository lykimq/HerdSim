 ...
 * Method default_config should not declare these; paper/custom merge them from
...
/** Short text for the selected method (mechanism / paper title). */
export function methodBlurb(method) {
...
export function presetSourceBlurb(
  presetId,
  { method, scenario, paperTaskLocked = true } = {},
) {
  ...
  if (presetId === "custom") {
    return "Customize scenario, sheep/dogs, experimental factors, and optional method or world parameters.";
  }
  const name = method?.name || "this method";
  const cfg = method?.default_config || {};
  const nSheep = cfg.n_sheep;
  const nDogs = cfg.n_shepherds;
  const counts =
    nSheep != null && nDogs != null ? `${nSheep} sheep, ${nDogs} dogs` : null;
  const mech = methodBlurb(method);
