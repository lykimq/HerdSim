/**
 * Short help text for Custom mode method params and world overrides.
...
/** Method / shared numeric keys shown under Method parameters. */
export const METHOD_PARAM_DESCRIPTIONS = {
  r_a: "Sheep-sheep repulsion distance (typical ~2). Also scales Collect/Drive stand-off and f(N). Larger = more personal space and more Collect; smaller = tighter flock and earlier Drive.",
  r_s: "Shepherd/dog detection range. Outside this, sheep graze; inside, they respond. Smaller = later pressure; larger = sheep react from farther away.",
  rs_weight:
    "Weight on shepherd repulsion in the sheep heading sum (typical ~1). Higher = sheep flee harder; lower = harder to push.",
  c: "Local centre-of-mass attraction weight (Strombom-family, typical ~1.05). Higher = stronger clustering in the neighbourhood.",
  inertia:
  ...
  K_s1: "Sheep-sheep repulsion gain. Higher = more personal space inside the flock.",
  K_s2: "Sheep alignment gain. Higher = stronger velocity matching among neighbours.",
  K_s3: "Sheep cohesion gain. Higher = stronger pull toward nearby sheep.",
  K_s4: "Sheep repulsion from dogs (usually large). Higher = sheep flee dogs more strongly.",
  K_f1: "Dog attraction to the target sheep. Higher = dogs close in faster.",
  K_f2: "Dog repulsion from the target sheep. Higher = less overshoot/collision with the target.",
  K_f3: "Dog repulsion from the goal. Higher = dogs stay behind the flock instead of entering the goal.",
  K_f4: "Dog-dog repulsion. Higher = wider arc; too low causes dogs to stack.",
  dt: "Kubo integration time step (> 0). Smaller = finer motion; larger = faster but coarser steps.",
  sheep_speed_max:
...
export const WORLD_PARAM_DESCRIPTIONS = {
  ...
  pen_center: "Containment pen centre [x, y]. Used by containment-style tasks.",
  pen_radius:
  ...
  gate_width: "Width of a choke-point gate. Narrower = harder passage.",
  gate_x:
...
/**
 * Description for method or world override keys.
 * For experimental factors, use factorFieldDescription from factors.js.
 */
export function paramFieldDescription(key) {
  if (!key) return "";
  return METHOD_PARAM_DESCRIPTIONS[key] || WORLD_PARAM_DESCRIPTIONS[key] || "";
}
...
export function setParamItemDescription(paramItem, text) {
  if (!paramItem) return;
  paramItem
    .querySelectorAll(":scope > .param-desc")
