...
A scenario defines the task, the world layout, and the success criterion for a simulation run. Any method can run in any scenario; scenarios and methods are fully independent.
...
For fair cross-model comparison, use **custom** (or Compare shared settings) to lock the same `n_sheep` and `n_shepherds` for every method on the same scenario. See `compare.md`.

There is no method-specific replication scenario. Models such as Kubo keep MATLAB-faithful force parameters in the method package; the shared scenario supplies the HerdSim world.

## What scenarios control

Scenarios own the world geometry: arena dimensions, goal or pen position and radius, obstacle placement, gate parameters (position, width, wall thickness), and containment thresholds. Methods run inside the world the scenario defines.
...
**First success tick** (in run reports and Experiments exports) is the tick at which the scenario success criterion first held. This is the primary outcome measure for comparing methods on a given scenario.
