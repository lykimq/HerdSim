...
Use Compare when you want to **watch** two methods at the same time, scrub both timelines, and read live metric deltas (A minus B).

It answers questions like: on this seed and layout, does method A gather faster, stay tighter, or reach the goal sooner tha method B?
...
Fair compare keeps the two sides matched so differences are mostly about the methods.
...
- Choose the **method** (and display options)
...
- Shepherd path needs care across method families. Kubo advances with a time step (`dt`); Strombom-style controllers move by displacement per tick. Path length is not always directly comparable.
...
|------|------|------------|
| Matched live A/B | Fair compare | Shared scenario, seed, N, M; methods on A and B; Init Both, Play Both |
