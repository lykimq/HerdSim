...
| Goal | Mode |
|------|------|
| Rank several methods on the same task and seeds | Compare methods |
...
Use this mode to answer: under matched conditions, which methods succeed more often, finish sooner, or keep a tighter flock?

1. Set Mode to **Compare methods**.
2. Tick the methods you want in the list.
3. Pick a **scenario** (Drive to Goal is the usual baseline).
4. Choose the settings source:
   - **Custom:** best for fair ranking. Lock the same sheep and dog counts for every method.
   - **Method (paper):** each method keeps its published counts. Good for replication, not for head-to-head ranking.
5. Enter **seeds** as a comma-separated list (for example `1, 2, 3, 4, 5`). More seeds give stabler rates and spreads.
6. Click **Run Experiment**.

Each summary row aggregates one method across those seeds: success and failure rates, tick timing on successful runs, trajectory summaries (cohesion, fragmentation, and related scores), and control efficiency. Failed trials can include failure labels in the export.

### Fair ranking in this tab

For a fair method ranking here:

- Same scenario for every selected method
...
There is no method checklist in factor-grid mode. Sheep model and dog controller rows (when present) select a matching method parameter bundle when HerdSim knows one.
...
**Summary table.** One row per method (compare mode) or per factor cell (grid mode). Hover column headers for short definitions. Prefer success rate and spreads (for example timing on successful runs) over a single lucky seed.
...
- For method ranking, prefer Custom counts and the same scenario for every method.
...
- Do not treat shepherd path as interchangeable across Kubo and Strombom-style methods without reading the export caveats.
...
- Methods: what each method package contains
