...
Metrics are computed at every tick from observable simulation state and are method-agnostic unless noted. They appear in the live metrics panel during Simulate and Compare runs, in the scrub history, and in Experiments exports.
...
| Metric | Definition |
|--------|------------|
| **Outlier Count** | Number of sheep beyond f(N) = `r_a` * N^(2/3) (optionally scaled by `collect_threshold_scale`) from the GCM. Same formula for every method so Compare/Experiments share one spread scale. For Collect/Drive controllers this matches the mode switch; for Kubo it is a report score only. Not "sheep outside the goal." |
...
Cohesion, GCM to Goal, Success Rate, Sheep in Goal, Fragmentation, Outlier Count, Min Separation, and Polarisation use the same definitions for every method on a shared seed and scenario.

**Shepherd Path** and tick counts are comparable across methods only when they share the same time-step convention. Strombom-family methods advance by a fixed displacement per tick; Kubo integrates over continuous `dt`. Path lengths are not directly comparable without normalising by the integration step.
