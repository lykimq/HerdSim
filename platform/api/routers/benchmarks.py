...
class BenchmarkRequest(BaseModel):
    methods: list[str] = Field(default_factory=lambda: ["strombom", "kubo"])
    ...
    if specs:
        ...
        if req.methods:
            if len(req.methods) != 1:
                raise HTTPException(
                    status_code=400,
                    detail="Param sweep with a method requires exactly one method",
                )
        ...
        raise HTTPException(status_code=400, detail="methods required")
    return specs
...
def benchmark_run(
    ...
    def event_stream():
        ...
        try:
            ...
            method_loop = list(req.methods) if req.methods else [None]
            total = len(method_loop) * len(req.seeds) * len(param_sets)
            index = 0
