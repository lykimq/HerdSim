"""N x D x seed budget grid runner with resume and provenance (Caps I1, I14)."""
...
from services.budget.layout import write_status
...
def _run_cell(cell: BudgetCell) -> dict[str, Any]:
    """Worker entry: run one trial via SimulationRunner (same stack as benchmarks)."""
    get_instrument(cell.instrument)
...
def expand_budget_grid(
    ...
    *,
    ...
    """Expand a protocol dict into BudgetCell jobs."""
    instruments = instruments or [protocol.get("baseline_method", "strombom_multi")]
...
def run_budget_grid(
    ...
    *,
    ...
    campaign_id: str = "budget_grid",
    max_workers: int | None = None,
    store_timeseries: bool = True,
    resume: bool = True,
) -> pd.DataFrame:
    """Run an N x D x seed campaign with resumable manifest and provenance."""
    out = Path(output_dir)
