...
Status / protocols / run plan: [progress_tracker.md](progress_tracker.md)  
...
status and protocol outcomes stay in the progress tracker -- keep them out of this
...
*herding method* or *method* for the dog algorithm; the code may still use
...
| Herding method | The shepherding algorithm that drives the dogs (e.g., `strombom_multi`, `kubo`) |
...
| RQ4 | Prevents method-local overgeneralisation | States which limits are method-invariant | Treat results as method-specific |
...
|---------|----------|-------------|
| A | Reliability maps R(N, D), frontier curves (D_min, D_overcrowd, D_max, B*), regime labels, provenance stamp | Phase 1 grid protocol |
| B | Per-(N, X₀, D) reliability, D_min(N, X₀) comparisons, predictor-comparison tables | Phase 2 X₀-manipulation protocol |
| C | Per-trial I_dir(t) and C(t) time series, regime-paired rank tests, mechanism-hypothesis verdicts | Phase 3 mechanism protocol |
| D | Transfer table: property × method → shared / shifted / absent | Phase 4 multi-method protocol |
| E | D_min(N, I) curves, substitution rates ΔD_min/ΔI, method-conditional notes | Phase 5 factor sweep |
...
**HerdSim must provide:** scaling grid runner with provenance; frontier (D_min, D_overcrowd, D_max, B*); regime labelling; Package A export.
...
**Why this RQ is valuable:** Results on one method are not scientific laws. Transfer labels (shared / shifted / absent) bound external validity and stop overclaiming.
...
**HerdSim must provide:** multi-method grid runs under identical τ, seeds, metrics; transfer table builder.
...
**Why this RQ is valuable:** If information substitutes for dogs at fixed R, the budget is multi-resource, not D-only. That is the scientific basis for P3. Substitution must be measured as ΔD_min / ΔI, and only for methods that consume the varied observation/communication factors.
...
**HerdSim must provide:** factor-sweep runner over existing observation/communication factors; substitution curves; method-conditional reporting when a factor is ignored by a herding method.
...
Machine-readable freeze: [../configs/canonical_grid.yaml](../configs/canonical_grid.yaml)  
Protocol subsets (with per-field rationale): [../configs/protocols/](../configs/protocols/)  
New protocol rule: every override needs a WHY comment -- see `protocols/README.md`.
...
**Future experiments:** any new `scaling/configs/protocols/*.yaml` must say why each
...
If we later want N=1-4, run a separate control protocol (individual-pursuit baseline)
...
| 5 | Information substitution | RQ5 | Package E | Phase 1 | C5a/C5b evaluable on methods that consume I factors |
...
|-----|--------|------------|------------------|--------|---------|
| I1 | RQ2, RQ6 | N × D × seed grid runner with resume + provenance | `scaling/services/scaling/runner.py`, `analysis/scaling/provenance.py` | built | A, F |
| I2 | RQ2 | Frontier extraction D_min, D_overcrowd, D_max, B* | `analysis/scaling/frontier.py` | built | A |
...
| I6 | RQ1 | State vs (N, D) predictor comparison | `analysis/scaling/predictors.py` | built | B |
...
| I8 | RQ3 | Mechanism hypothesis tests | `analysis/scaling/mechanism.py` | built | C |
| I9 | RQ4 | Locked multi-method protocols + transfer table | `analysis/scaling/transfer.py` | built | D |
| I10 | RQ5 | Factor sweeps (obs, range, communication) + substitution curves | `scaling/scripts/run_factor_sweep.py`, `analysis/scaling/substitution.py` | built | E |
| I11 | RQ6 | Scaling model fits | `analysis/scaling/fits.py` | built | F |
| I12 | RQ7 | Early-warning features and evaluation | `analysis/scaling/early_warning.py` | built | G |
| I13 | S8 | Canonical protocol config + dossier export | `scaling/configs/canonical_grid.yaml`, `analysis/scaling/export.py` | built | all |
| I14 | RQ3, RQ7 | Per-trial time-series storage (Parquet) | `scaling/services/scaling/runner.py` (write path) | built | C, G |

**Already available (do not rebuild):** cohesion, fragmentation, outlier_count metrics; `iter_one_trial` / Experiments path; observation modes and communication factor injection in the simulation runner; method presets listed in RQ4; failure taxonomy via benchmark runner.
...
Running protocols without the matching Cap does not count as answering the RQ.
...
**RQ1 gate:** unit tests must verify that sampled layouts differ on the intended metric (cohesion for compact/wide; fragmentation for split; outlier_count for outlier_rich) before any claim-grade protocol.
...
|-----------|------|---------|
| Canonical grid config | `scaling/configs/canonical_grid.yaml` [NEW] | Machine-readable protocol defaults |
| Scaling grid runner | `scaling/services/scaling/runner.py` [NEW] | N×D×seed protocols with resumability and provenance |
| Provenance stamps | `analysis/scaling/provenance.py` [NEW] | Git hash, config hash, seed list, timestamps |
| Frontier extraction | `analysis/scaling/frontier.py` [NEW] | D_min, D_overcrowd, D_max, B* |
| Regime labelling | `analysis/scaling/regimes.py` [NEW] | Assigns regime labels to (N, D) cells |
| Export / dossier | `analysis/scaling/export.py` [NEW] | CSV + markdown report per package |
...
(includes N, D, layout, seed, method, and any info-factor suffixes).

```
scaling/results/phase{k}/{protocol_slug}/timeseries/<cell_key>.parquet
```

Example: `N50_D2_Lcompact_S2026_Mstrombom_multi.parquet`
...
**Implementation:** After each trial completes, write `result.history` to Parquet under `timeseries/`. Also write `status.json` with planned/done counts. Protocol subset YAML is copied to `protocol.yaml` in the output folder.

#### 11.5 Analysis modules (Phases 2–7)

| Module | File | Phase | Serves |
|--------|------|-------|--------|
| Transfer table builder | `analysis/scaling/transfer.py` [NEW] | 4 | RQ4 |
| Mechanism hypothesis tests | `analysis/scaling/mechanism.py` [NEW] | 3 | RQ3 |
| Substitution curves | `analysis/scaling/substitution.py` [NEW] | 5 | RQ5 |
| Scaling model fitting | `analysis/scaling/fits.py` [NEW] | 6 | RQ6 |
| Early-warning models | `analysis/scaling/early_warning.py` [NEW] | 7 | RQ7 |
| Predictor evaluation | `analysis/scaling/predictors.py` [NEW] | 2 | RQ1, RQ6 |
...
**Design:** The scaling grid runner uses `concurrent.futures.ProcessPoolExecutor` with configurable `max_workers` (default: `os.cpu_count() - 1`).
...
- For cluster execution, provide a `scaling/scripts/submit_slurm.py` template that partitions the grid into array-job tasks. This is optional and not gated by any Phase; add it when a cluster is available.
...
|------|--------|-------|--------|
| Reliability heatmap R(N, D) | `analysis/scaling/plots.py` | 1 | RQ2, debugging |
| Frontier overlay D_min(N) with CI | `analysis/scaling/plots.py` | 1 | RQ2, RQ6 |
| I_dir time-series overlay (efficient vs overcrowding) | `analysis/scaling/plots.py` | 3 | RQ3 |
| Substitution curve D_min vs I_level | `analysis/scaling/plots.py` | 5 | RQ5 |
| Scaling fits (data + candidate curves) | `analysis/scaling/plots.py` | 6 | RQ6 |
| ROC curves (state-based vs baseline) | `analysis/scaling/plots.py` | 7 | RQ7 |
...
| Script | Purpose |
|--------|---------|
| `scaling/scripts/run_grid.py` | Run single or multi-method N×D campaigns |
| `scaling/scripts/run_factor_sweep.py` | Sensing/communication sweeps for RQ5 |
| `scaling/scripts/analyse.py` | Extract frontiers, regimes, transfer tables from completed protocols |
...
│   │   └── layout.py          # path conventions + protocol helpers
│   └── experiments/           # UI Experiments / factor-grid engine
├── scaling/configs/
│   ├── canonical_grid.yaml    # frozen Section 8 protocol
│   └── protocols/             # per-run subsets (pilot, scout, state, ...)
├── scaling/scripts/
│   ├── run_grid.py
│   ├── run_factor_sweep.py
│   └── analyse.py
├── scaling/results/            # campaign run data (kept in git)
│   ├── README.md
│   └── phase{k}/{protocol}/
│       ├── protocol.yaml
│       ├── manifest.jsonl
...
└── tests/backend/correctness/
    └── test_scaling_stack.py
```

Operator facade: `scaling/Makefile` (`make -C scaling help`, or `make scaling-help`). Targets: `scaling-pilot`, `scaling-scout`, `scaling-pilot-state`, `scaling-factor-sweep`, `scaling-analyse`.
...
| `methods/*` | We study existing methods as-is |
...
| 1.2 | `scaling_runner.py` (single method, with parallelisation + manifest) | I1, I14 | Integration test (tiny grid) | 1 |
...
| 1.7 | **Run Phase 1 protocol** | -- | C2 evaluable | 1 |
...
| 2.3 | **Run Phase 2 protocol** | -- | C1a/C1b evaluable | 2 |
...
| 3.3 | **Run Phase 3 protocol** | -- | C3 evaluable | 3 |
| 4.1 | Multi-method grid runner | I9 | Integration test | 4 |
| 4.2 | `transfer.py` | I9 | Unit test | 4 |
| 4.3 | **Run Phase 4 protocol** | -- | C4 evaluable | 4 |
| 5.1 | Factor-sweep runner | I10 | Integration test | 5 |
| 5.2 | `substitution.py` | I10 | Unit test | 5 |
| 5.3 | **Run Phase 5 protocol** | -- | C5a/C5b evaluable | 5 |
...
| **Herding-method fidelity** | Simulated methods may not capture all phenomena of physical robots (communication latency, actuator noise, GPS error). | Scope claims to the simulated domain. RQ5 partially tests information degradation. Physical validation is out of scope for this program. |
...
| 2026-09-17 | Terminology de-risked: "herdability" replaces "controllability"; "herding method" / "method" replaces "controller" in prose |
...
| 2026-09-18 | Results layout cleanup: `phase{k}/{slug}/`, `packages/{a-g}/`, protocol YAMLs, `status.json`, timeseries stem = cell key, `artefacts.json`, tracked `scaling/results/README.md` |
| 2026-09-18 | Protocol rationale added: comments on all budget YAML fields + main plan Section 8.1; protocols/README checklist for future experiments |
...
3. Transfer methods: strombom_multi, kubo, fat, communication_free.
...
- 2026-09-17: Caps I1-I14 implemented in HerdSim (metrics, analysis/scaling, scaling/services/scaling/runner, scaling/configs, scaling/scripts). Phase done-when = claim-evaluable tooling + unit tests; full scout protocols remain operator runs.
