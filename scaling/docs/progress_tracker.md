...
| [main_scaling_plan.md](main_scaling_plan.md) | Detailed plan |
| [REPORT_TEMPLATE.md](REPORT_TEMPLATE.md) | Human REPORT.md template |
| [results/README.md](../results/README.md) | Results layout conventions |
| This file | Where we are, what ran, what is next; hardware + ordered run plan |

Protocol subsets: `scaling/configs/protocols/` (each field has a WHY comment; new protocols must follow `protocols/README.md`).  
Operator entry: `make -C scaling help` (or `make scaling-help`).
Protocol rationale: main plan Section 8.1 and comments in `scaling/configs/canonical_grid.yaml`.

Wall-clock times depend on the machine. For claim-grade protocols, note which host
...
| Item | Status |
|------|--------|
| Protocol (Section 8) | DONE -- frozen 2026-09-17 as `scaling_v1` |
...
| IN PROGRESS | Scout/claim protocol running or partial |
...
| 1 | Herdability maps | RQ2 (+ RQ6 data) | A | IN PROGRESS | Pilot + partial scout historically; compact R=1, D_min=1. Path: `scaling/results/phase1/{pilot,scout}/` |
| 2 | State beyond N | RQ1 | B | NOT STARTED | `make scaling-pilot-state` -> `scaling/results/phase2/pilot_state/` |
...
| 5 | Information substitution | RQ5 | E | NOT STARTED | `make scaling-factor-sweep` -> `scaling/results/phase5/factor_sweep/` |
...
1. `make -C scaling scaling-pilot` -- confirm Package A reports/figures after layout cleanup.
2. `make -C scaling scaling-scout WORKERS=12` -- or skip finishing if compact remains flat (Gate A in Section 7).
3. `make -C scaling scaling-pilot-state` -- then add a Phase-2 scout YAML if X0 metrics separate.
...
Re-measure and add a row if the primary box changes or a protocol runs elsewhere.
...
| GPU | none (HerdSim scaling sims are CPU-bound) |
| CPU governor | `powersave` at snapshot -- switch to `performance` for long protocols |
...
- For claim-grade protocols, copy this table into the protocol `REPORT.md` (or at least note host + `WORKERS` + governor).
...
## 7. Ordered run plan (one protocol at a time)

Finish the original program (Size -> Structure -> Mechanism -> Generality, then
follow-ons) without one giant grid. Run **one protocol**, write `REPORT.md`, update
...
Protocol freeze (`scaling_v1`). Section 8 frozen 2026-09-17.

### Wave 1 -- Size map (RQ2 data, feed RQ6)

| # | Campaign | Command / config | ~Time* | Done when |
|---|----------|------------------|--------|-----------|
| 1 | Smoke pilot | `make -C scaling scaling-pilot` (optional `SCALING_MAX_TICKS=3000`) | minutes | Pipeline + Package A + figures OK |
| 2 | Phase-1 scout (compact) | `make -C scaling scaling-scout WORKERS=8-16` | ~1--4 h | R(N,D), frontier, regimes exported |
...
| # | Campaign | Command / config | ~Time* | Done when |
|---|----------|------------------|--------|-----------|
| 3 | State smoke | `make -C scaling scaling-pilot-state` | tens of min | All 4 X0 differ on metrics; Package B path runs |
| 4 | State scout | New protocol YAML: N/D like phase1 scout (or smaller), **all 4 X0**, 30 seeds, baseline only | ~4--12 h (longer if hard) | Path to C1a; D_min differs by layout |
...
| 13 | (Optional) Paper cross-check | later validation | separate protocol |
...
1. One protocol -> `REPORT.md` -> update this tracker -> next.
...
\*Hard layouts that often hit T0=10000 can multiply wall time; still fine one protocol
...
Designs: main plan Section 10.1. Here only protocol use.

| Cap | Used in a protocol? | Note |
|-----|---------------------|------|
...
| I10 factor sweep | no | `scaling-factor-sweep` ready |
...
## 9. Protocol drift (latest primary protocol)
...
Run data under `scaling/results/` is kept in git once committed.
...
| Path | `scaling/results/phase1/scout/` |
...
Resume: `make -C scaling scaling-scout WORKERS=12` (keep `manifest.jsonl`).

### `scaling_pilot` / `phase1_pilot` (2026-09-17) -- SMOKE

Path: `scaling/results/phase1/pilot/`. Same qualitative picture on small N.
...
| 2026-09-18 | layout cleanup | -- | -- | Standardized paths + protocol YAMLs | `scaling/results/README.md` |
...
- [ ] Section 8 Cap use
- [ ] Section 9 drift
- [ ] Section 10 primary protocol + REPORT.md (note host / WORKERS)
- [ ] Section 11 log row
- [ ] Last updated date
