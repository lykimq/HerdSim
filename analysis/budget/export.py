...
from analysis.budget.plots import (
    save_frontier_curve,
    save_regime_counts,
    ...
    if "initial_layout" in rates.columns and rates["initial_layout"].nunique() > 1:
        for layout, sub in rates.groupby("initial_layout", dropna=False):
            ...
            saved = save_reliability_heatmap(
                ...
                path,
                title=f"R(N, D) -- {campaign_id} / {layout}",
            )
...
def export_package_a(
    ...
    *,
    ...
    instruments = (
        _unique_sorted(trials["instrument"]) if "instrument" in trials.columns else []
    )
    layouts = (
    ...
    lines = [
        ...
        "",
        "Auto-generated evidence package. Interpretation belongs in the campaign `REPORT.md`.",
        "",
        "## Setup",
        "",
        f"- Protocol id: `{protocol.get('protocol_id', 'unknown')}`",
        f"- Reliability theta: {theta}",
        f"- Instruments: {instruments or ['(not in trials.csv)']}",
