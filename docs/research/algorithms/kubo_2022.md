...
Kubo is the main non-Strombom instrument in HerdSim, so you can compare force-based herding with Collect / Drive on the same scenarios, seeds, and metrics.
...
| Instrument | Integration | "Farthest" meaning |
|------------|-------------|--------------------|
...
**Implemented model:** force decomposition (sheep K_s1..K_s4, dog K_f1..K_f4), sensing `radius`, `dt` integration, speed clamps, and dog update after sheep for the tick. The HerdSim preset uses 40 sheep and 4 dogs; use custom counts for a controlled cross-instrument comparison.
