...
| Item | Default | Status |
|------|---------|--------|
| Task τ₀ | `drive_to_goal` | pending confirm |
| Reliability θ | 0.90 (also report 0.50, 0.70) | pending confirm |
...
| X₀ families | compact, wide, split, outlier_rich | pending confirm |
| Time limit T₀ | 10,000 ticks (overrides scenario default 3,000) | pending confirm |
| Extended time limit T₁ | 20,000 ticks (for C2b hard-ceiling test) | pending confirm |
...
|-----|--------|------------|------------------|--------|---------|
| I1 | RQ2, RQ6 | N × D × seed grid runner with resume + provenance | `api/budget_runner.py`, `analysis/budget/provenance.py` | to build | A, F |
| I2 | RQ2 | Frontier extraction D_min, D_overcrowd, D_max, B* | `analysis/budget/frontier.py` | to build (extends `analysis/herdability.py`) | A |
| I3 | RQ2 | Regime labelling | `analysis/budget/regimes.py` | to build | A |
| I4 | RQ1, RQ6, RQ7 | Mean-spread and extent metrics | `metrics/mean_spread.py`, `metrics/extent.py` | to build | B, F, G |
| I5 | RQ1 | X₀ generators + scenario wiring | `core/x0_generators.py`; update `drive_to_goal` + `INITIAL_LAYOUTS` | to build | B |
| I6 | RQ1 | State vs (N, D) predictor comparison | `analysis/budget/predictors.py` | to build | B |
| I7 | RQ3, RQ7 | Interference I_dir and coverage C metrics | `metrics/shepherd_interference.py`, `metrics/shepherd_coverage.py` | to build | C, G |
| I8 | RQ3 | Mechanism hypothesis tests | `analysis/budget/mechanism.py` | to build | C |
| I9 | RQ4 | Locked multi-instrument campaigns + transfer table | `analysis/budget/transfer.py` | to build | D |
| I10 | RQ5 | Factor sweeps (obs, range, communication) + substitution curves | `scripts/budget/run_factor_sweep.py`, `analysis/budget/substitution.py` | to build (factors already exist in core) | E |
| I11 | RQ6 | Scaling model fits | `analysis/budget/scaling.py` | to build | F |
| I12 | RQ7 | Early-warning features and evaluation | `analysis/budget/early_warning.py` | to build | G |
| I13 | S8 | Canonical protocol config + dossier export | `configs/budget/canonical_grid.yaml`, `analysis/budget/export.py` | to build | all |
| I14 | RQ3, RQ7 | Per-trial time-series storage (Parquet) | `api/budget_runner.py` (write path) | to build | C, G |
...
| pending | Freeze protocol (Section 8) |

### 18. Open decisions before protocol freeze

1. **Task:** Confirm `drive_to_goal` as τ₀ (recommended for Phase 1). Optional later robustness task: `narrow_gate` — not required to justify RQ1–RQ3.
2. **Reliability threshold:** θ = 0.90?
3. **Transfer instrument set:** strombom_multi, kubo, fat, communication_free?
4. **Flock sizes:** {25, 50, 75, 100, 150, 200, 300, 400}?
5. **Time limit:** T₀ = 10,000 ticks? T₁ = 20,000 ticks for C2b?
6. **X₀ families:** compact, wide, split, outlier_rich — sufficient for C1a?
7. **RQ7 parameters:** k = 500 ticks, w = 200 ticks?
8. **Term in titles/abstracts:** use "herdability" in paper title and abstract; use "operational herdability" at first mention; avoid bare "controllability" in public prose.
9. **First cut:** confirm minimum publishable unit = RQ1+RQ2+RQ3+S8 before building Phase 4+ tooling.

When confirmed, update Section 8 to status **frozen** and begin Phase 1 implementation only for Caps I1–I4, I13, and I14 (plus I5 when starting Phase 2).
