...
The platform is built around named instruments (a sheep model plus a dog controller, with paper-style defaults) and shared metrics. That combination lets you measure when herding works, when it fails, and how methods compare when flock size, sensing, scenario, and seeds are held fair.

## What you can do

- **Simulate.** Run one instrument live. Scrub history, inspect agents, and download a run report.
...
- **NetLogo.** For instruments that have a desktop twin, open the NetLogo model from here. That twin is not the browser simulation.
...
- **Instrument:** a named ready-made package (how sheep move plus how dogs decide), with paper-style defaults.
- **Scenario:** the task and world layout (arena, goal or pen, obstacles, success rule). Examples: Drive to Goal, Obstacle Course.
- **Seed:** the random draw for start positions and chance events. Same instrument, scenario, seed, and settings give the same replay.
...
- **Paper defaults:** published-style sheep and dog counts and parameters for an instrument. Good for replication. Not always a fair head-to-head.
...
- **Metric:** a number computed each tick or summarised over a trial. Definitions are shared across instruments.

### Agents and flock

- **Sheep:** flock agents that move, graze, and usually flee dogs.
- **Shepherd / dog:** the herding agent. Naming differs by instrument, but the role in the UI is the same.
...
An instrument is a ready-made herding setup: how sheep move plus how dogs decide where to go, with sensible defaults. Pick one in Simulate, Compare, or Experiments. You can still change sensing, counts, noise, and other factors around it. The terms above apply to the list below.
...
You choose an instrument, a scenario, a seed, and optional factors. Same instrument, scenario, and seed replay the same way. Metrics use the same definitions across instruments so Compare and Experiments stay meaningful.
...
**Seeded randomness.** Same instrument, scenario, seed, and settings give the same replay.
